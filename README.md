...
The map will display earthquake locations with circle markers sized by magnitude. A color scale will indicate shallow and deep quakes. On click, popups will show timestamp, location, magnitude, and depth. A slider will filter the date range from 1-30 days. Users can toggle between light, dark, and satellite map layers using controls provided by MapBox. There will also be options to display tectonic plate boundaries and filter only major quakes above magnitude 7 with prominent red circle markers.
